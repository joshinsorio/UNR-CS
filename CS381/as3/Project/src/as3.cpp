...
as3::as3(void): mSurfaceHeight(-3.5), mArrowUPressed(false), mArrowDPressed(false), mArrowRPressed(false), mArrowLPressed(false), mPgUPressed(false), mPgDPressed(false), mCameraVelocity(0.f), m_entity_manager(nullptr)
{
}
...
void as3::createScene(void)
{
	...
	//create 5 CG47
//	for (unsigned int i = 0; i < 5; ++i)
	m_entity_manager->CreateEntityOfType(Entity381::CG47, Ogre::Vector3(0 * spacing, mSurfaceHeight + 1, -600));

	//create 5 CVN75
//	for (unsigned int i = 0; i < 5; ++i)
	m_entity_manager->CreateEntityOfType(Entity381::CVN75, Ogre::Vector3(1 * spacing, mSurfaceHeight + 1, -600));

	//create TUG
	m_entity_manager->CreateEntityOfType(Entity381::TUG, Ogre::Vector3(2 * spacing, mSurfaceHeight + 1, -600));

	//create DDG51
	m_entity_manager->CreateEntityOfType(Entity381::DDG51, Ogre::Vector3(3 * spacing, mSurfaceHeight + 1, -600));

	//create CIGARETTE
	m_entity_manager->CreateEntityOfType(Entity381::CIGARETTE, Ogre::Vector3(4 * spacing, mSurfaceHeight + 1, -600));


//	//PolyPlane
//	Ogre::Entity* planeEntity = mSceneMgr->createEntity("ddg51-v2.mesh");
...
bool as3::frameRenderingQueued(const Ogre::FrameEvent &e) {
...
bool as3::processUnbufferedInput(const Ogre::FrameEvent& e)
{
	//Update Objects
...
bool as3::keyPressed(const OIS::KeyEvent &arg) {
...
//
//		case OIS::KC_SPACE:
//			m_entity_manager->resetSelectedSpeedAndHeading();
//			break;
...
bool as3::keyReleased(const OIS::KeyEvent &arg) {
	switch (arg.key) {
	case OIS::KC_DOWN:
